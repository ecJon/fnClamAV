...
> 项目中存在 `rust-cgi/` 目录，这是一个独立的 Rust CGI 实现。
> 它不依赖后台 daemon，直接在 CGI 进程中处理请求和扫描。
> 目前生产环境未使用，保留作为轻量部署的备用方案。

```
┌────────────────────────────────────────────────────────────┐
│                 rust-cgi (Rust CGI 程序)                    │
│         自包含的 API 处理 + ClamAV 调用                      │
│         无需后台 daemon，适合轻量部署                        │
└────────────────────────────────────────────────────────────┘
```
...
| **CGI 层（生产）** | Bash Shell (`app/ui/index.cgi`) | API 请求转发到 Rust daemon，静态文件服务 |
| **CGI 层（备用）** | Rust (`rust-cgi/`) | 独立 CGI 实现，无 daemon 依赖，轻量部署 |
