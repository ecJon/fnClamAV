...
**采用方案A：Rust 调用 ClamAV 二进制命令**

| 对比项 | 方案A（采用） | 方案B（未采用） |
|--------|--------------|----------------|
| **集成方式** | Rust 调用 clamscan/freshclam 二进制 | 动态链接 libclamav.so |
| **第三方依赖** | ✅ 无 | ❌ 需要 clamav-rs 等 FFI 库 |
| **实现复杂度** | ✅ 简单 | ❌ 中等 |
...
**决策理由**：
- 不依赖第三方 Rust FFI 库，维护更简单
- ClamAV 原版编译，无需修改源码
- 更新 ClamAV 版本时只需替换二进制文件

### 3.4.2 二进制文件布局

```
TRIM_APPDEST/bin/               # 应用可执行文件目录
├── clamscan                    # ClamAV 扫描命令
└── freshclam                   # 病毒库更新命令

$DATA_SHARE/clamav/             # 病毒库目录（数据共享目录）
├── daily.cld                   # 病毒库主文件
├── main.cld                    # 病毒库主文件
└── bytecode.cld                # 病毒库字节码
```
...
// 环境变量获取的路径
// let app_dest = env::var("TRIM_APPDEST").unwrap();     // 应用可执行文件目录
// let data_share = env::var("TRIM_DATA_SHARE_PATHS").unwrap(); // 数据共享目录
// let db_dir = format!("{}/clamav", data_share.split(':').next().unwrap());

// 扫描指定路径
fn scan_path(path: &str, db_dir: &str) -> Result<ScanResult, Error> {
    let output = Command::new("${TRIM_APPDEST}/bin/clamscan")
        .arg(format!("--database={}", db_dir))
        .arg("--recursive")           // 递归扫描
        .arg("--infected")             // 只显示感染文件
        .arg("--suppress-ok-results")  // 不显示正常文件
        .arg(path)
        .output()?;
...
### 3.4.4 病毒库更新调用示例（Rust）

```rust
// 更新病毒库
fn update_signatures(db_dir: &str) -> Result<UpdateResult, Error> {
    let output = Command::new("${TRIM_APPDEST}/bin/freshclam")
        .arg(format!("--datadir={}", db_dir))
        .arg("--stdout")               // 输出到标准输出
        .output()?;
...
**支持操作**：开始扫描、停止扫描
**暂不支持**：暂停、继续（clamscan 一次性运行，不支持中断后恢复）

**停止扫描实现**：
```rust
// 通过 kill 子进程实现停止
fn stop_scan(child: &mut Child) -> Result<(), Error> {
...
1. **预估总文件数**（在扫描开始前）
   ```rust
   // 使用 find 命令快速统计文件数
   fn estimate_file_count(path: &str) -> usize {
...
2. **实时计算进度**
   ...
   let total_estimate = estimate_file_count(path);

   for line in reader.lines() {
       if line.contains("OK") || line.contains("FOUND") {
           scanned_count += 1;
           let percent = (scanned_count as f32 / total_estimate as f32) * 100.0;
           update_progress(percent, &line);
       }
   }
   ```
...
3. **显示格式**
   ```
   扫描进度: 45% (约 1,234 / 2,700 文件)
   当前文件: /data/documents/file.pdf
   发现威胁: 3
   ```

**注意**：
- 预估值可能不准确（实际扫描会跳过某些文件类型）
- 扫描完成后显示准确的统计数据
...
# 配置编译选项
./configure --prefix=/usr/local --disable-clamav --disable-milter
...
- [x] 确定Rust服务与ClamAV的集成方案（采用二进制调用）
- [x] 扫描控制方案设计（仅支持停止，不支持暂停/继续）
- [x] 扫描进度估算方案（基于 find 预估文件数）
...
| 日期 | 版本 | 变更内容 | 作者 |
|------|------|----------|------|
| 2026-02-10 | 0.8 | 再次核对飞牛规范：修正 manifest 中 arch→platform，添加 app/ui/config 应用入口配置示例 | Claude |
| 2026-02-10 | 0.7 | 核对飞牛开发者文档规范，更新数据存储规范（使用 TRIM_* 环境变量）、更新应用打包结构、应用名称改为 fnnas.clamav | Claude |
| 2026-02-10 | 0.6 | 完成所有设计方案（定时更新、全盘扫描、编译打包、架构图更新），添加设计完成摘要 | Claude |
| 2026-02-10 | 0.5 | 完成隔离区管理方案设计（目录结构、处理流程、威胁/隔离区 API） | Claude |
| 2026-02-10 | 0.4 | 完善 API 接口设计（更新、配置、错误码），完成数据库表结构设计 | Claude |
| 2026-02-10 | 0.3 | 确定扫描控制方案（不支持暂停/继续），完成进度估算方案，完善扫描 API 设计 | Claude |
| 2026-02-10 | 0.2 | 确定ClamAV集成方案（二进制调用方式） | Claude |
| 2026-02-10 | 0.1 | 初始版本，完成需求讨论和架构设计 | Claude |
