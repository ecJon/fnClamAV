...
| **扫描功能** | - 全盘扫描：扫描所有挂载的硬盘数据<br>- 自定义扫描：用户选择/输入路径<br>- 扫描进度实时显示：百分比 + 当前文件 + 威胁数 + 扫描速率 |
| **扫描控制** | - 后台扫描：关闭窗口不打断<br>- 支持停止操作<br>- 重新进入应用可恢复扫描状态 |
| **威胁处理** | - 设置中可选：自动隔离 / 自动删除<br>- 威胁列表显示处理状态（已忽略/已隔离/已删除） |
...
└──────────┼──────────┼──────────┼──────────┼───────────────┘
           ...
           ▼          ▼          ▼          ▼
┌────────────────────────────────────────────────────────────┐
│            app/ui/index.cgi (Bash Shell)                    │
│         API 请求转发 → Rust Daemon :8899                    │
│         静态文件 → 直接返回                                  │
└───────────────────────────┬─────────────────────────────────┘
                            │ HTTP
                            ▼
┌────────────────────────────────────────────────────────────┐
│              Rust Daemon (127.0.0.1:8899)                   │
...
| 组件 | 技术 | 职责 |
|------|------|------|
| **前端** | Vue.js 3 | 用户界面，简洁风格 |
| **CGI 层** | Bash Shell (`app/ui/index.cgi`) | API 请求转发到 Rust daemon，静态文件服务 |
| **Rust Daemon** | Rust + Axum | 后台扫描服务、更新管理、状态持久（端口 8899） |
| **ClamAV** | C + Rust FFI | 病毒扫描引擎 (libclamav.so) |
| **数据库** | SQLite | 扫描历史、威胁记录、配置存储 |
...
|------|----------|
| 前端 | Vue.js 3 + Vite |
| CGI | Bash Script + curl |
| 后端 Daemon | Rust (Axum) |
| 数据库 | SQLite (rusqlite) |
| 杀毒引擎 | ClamAV 1.5.1 (libclamav.so + FFI) |
...
**cl_scanfile_ex 返回值说明**：
- `cl_scanfile_ex` **不返回 CL_VIRUS**，即使检测到病毒
- 必须检查 `verdict_out` 参数来判断检测结果
- `cl_verdict_t` 值：
  - `CL_VERDICT_NOTHING_FOUND = 0` - 未发现
  - `CL_VERDICT_TRUSTED = 1` - 可信
  - `CL_VERDICT_STRONG_INDICATOR = 2` - 强指标（病毒）
  - `CL_VERDICT_POTENTIALLY_UNWANTED = 3` - 潜在威胁（PUA）
...
### 3.4.3 扫描流程设计（两线程 + EMA 模式）

**架构设计**：

采用两线程模型，参考 Windows Defender、McAfee 等主流杀毒软件的实现方式：
...
| 变量 | 类型 | 说明 |
|------|------|------|
| `discovered_count` | AtomicU32 | 已发现的文件总数 |
| `scanned_count` | AtomicU32 | 已扫描的文件数 |
| `threats_count` | AtomicU32 | 发现的威胁数 |
| `discovery_complete` | AtomicBool | 发现线程是否完成 |
| `cancelled` | AtomicBool | 是否取消扫描 |

**EMA 速率计算**：

使用指数移动平均（Exponential Moving Average）计算扫描速率：

```
公式: new_ema = α × instant_rate + (1 - α) × old_ema

其中:
- α = 0.3 (平滑系数)
- instant_rate = scanned_files / elapsed_seconds
```
...
pub struct ScanProgress {
    pub percent: ProgressPercent,           // 进度百分比
    pub scanned_files: ScannedFiles,        // 已扫描文件数
    pub total_files: TotalFiles,            // 估算总数（= discovered）
    pub threats_found: ThreatsFound,        // 发现的威胁数
    pub current_file: Option<FilePath>,     // 当前扫描文件
    pub discovered_files: DiscoveredFiles,  // 已发现的文件总数
    pub scan_rate: Option<ScanRate>,        // 扫描速率（文件/秒）
}
```
...
| 状态 | 显示 |
|------|------|
| 扫描中 | `已扫描 70 / 8095 文件 (5.3 文件/秒)` |
| 完成后 | `扫描完成：8095 个文件` |

**优势**：

1. **即时反馈**：用户立即看到进度，不需要等待文件遍历完成
2. **准确的总数**：发现线程持续更新总数，进度百分比逐渐精确
3. **速率显示**：EMA 平滑处理，显示稳定的扫描速率
4. **内存高效**：使用通道传递文件路径，不需要预收集所有文件
...
| 操作 | 功能描述 | 实现方式 |
|------|----------|----------|
| 开始扫描 | 启动新的扫描任务 | 启动发现线程和扫描线程 |
| 停止扫描 | 终止当前扫描 | 设置 cancelled 原子标志，两线程优雅退出 |
...
```
扫描线程 ──────回调─────▶ Rust Service ──────轮询─────▶ 前端

进度更新字段:
├── scanned_files     - 已扫描文件数
├── discovered_files  - 已发现文件数（实时更新）
├── total_files       - 当前已知总数（= discovered）
├── scan_rate         - 扫描速率（文件/秒）
├── current_file      - 当前扫描文件
...
**进度更新频率**：
- 扫描线程每 100ms 更新一次进度（避免过于频繁）
- 前端每 2 秒轮询一次 `/api/scan/status`
- 扫描完成后自动刷新威胁列表
...
CREATE TABLE threat_records (
    ...
    action_taken TEXT,           -- 处理动作：ignored/quarantined/deleted
    action_time INTEGER,         -- 处理时间
    original_location TEXT,      -- 原始位置（隔离时使用）
    file_hash TEXT               -- 文件哈希
);
```
...
```json
{
  ...
  "items": [
    {
      "id": 1,
      "scan_id": "scan_20260214_120000",
      "file_path": "/data/infected.exe",
      "virus_name": "Trojan.Generic",
      "detected_time": 1707907200,
      "action_taken": null,          // null = 待处理
      "action_time": null
    },
    {
      "id": 2,
      "scan_id": "scan_20260214_120000",
      "file_path": "/data/virus.doc",
      "virus_name": "Macro.Virus",
      "detected_time": 1707907200,
      "action_taken": "ignored",     // 已处理
      "action_time": 1707907800
    }
  ]
}
```
...
| action_taken | 显示效果 | 操作按钮 |
|--------------|----------|----------|
| `null` | 无标签 | 显示（隔离/忽略） |
| `"ignored"` | 灰色标签 "已忽略" | 隐藏 |
| `"quarantined"` | 黄色标签 "已隔离" | 隐藏 |
| `"deleted"` | 红色标签 "已删除" | 隐藏 |
...
| 日期 | 版本 | 变更内容 | 作者 |
|------|------|----------|------|
| 2026-02-13 | 1.1 | 更新状态为"已实现"；修正 CGI 层描述：生产环境使用 Bash CGI 转发到 Rust daemon，补充 rust-cgi 备用方案说明；更新技术栈和核心组件表 | Claude |
